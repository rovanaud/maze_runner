...
void keyframe_structure::initialize(numarray<vec3> const& key_positions_arg, numarray<float> const& key_times_arg)
{
	...
	//sphere_current.material.color = { 1,1,0 };
	draw_spider(sphere_current); 
...
void keyframe_structure::display_current_position(vec3 const& p, environment_structure& environment)
{
	...
	sphere_current["body"].transform_local.translation = p;


	sphere_current["head"].transform_local.rotation = rotation_transform::from_axis_angle({ 0,1,0 }, 0.3 * cos(-2 * timer.t));
	sphere_current["foot_1"].transform_local.rotation = rotation_transform::from_axis_angle({ 0,1, 1 }, 3 + 0.4 * cos(-3 * timer.t));
	sphere_current["foot_2"].transform_local.rotation = rotation_transform::from_axis_angle({ 0,1, 1 }, 3 + 0.4 * cos(-3 * timer.t));
	sphere_current["foot_3"].transform_local.rotation = rotation_transform::from_axis_angle({ 0,1, 1 }, 3 + 0.4 * cos(-3 * timer.t));
	sphere_current["foot_4"].transform_local.rotation = rotation_transform::from_axis_angle({ 0,-1, 1 }, 3 + 0.4 * cos(-3 * timer.t));
	sphere_current["foot_5"].transform_local.rotation = rotation_transform::from_axis_angle({ 0,-1, 1 }, 3 + 0.4 * cos(-3 * timer.t));
	sphere_current["foot_6"].transform_local.rotation = rotation_transform::from_axis_angle({ 0,-1, 1 }, 3 + 0.4 * cos(-3 * timer.t));

	sphere_current.update_local_to_global_coordinates();
